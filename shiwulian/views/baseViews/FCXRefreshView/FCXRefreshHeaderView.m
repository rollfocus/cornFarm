...
- (void)addRefreshContentView {
    ...
    _statusLabel.frame = CGRectMake(0, 30, screenWidth, 20);
...
//    [self addSubview:_timeLabel];
    ...
    _arrowImage.frame = CGRectMake(screenWidth/2.0 - 60, (FCXLoadingOffsetHeight - 40)/2.0 + 5, 15, 40);
//    [self addSubview:_arrowImage];
    
    //转圈动画
    _activityView = [[UIActivityIndicatorView alloc] initWithActivityIndicatorStyle:UIActivityIndicatorViewStyleGray];
    CGRect frame = _arrowImage.frame;
    frame.origin.y -= 2;
    ...
    //设置转圈为中心位置
    CGFloat width = 15.0;
    CGFloat posX = (screenWidth - width) / 2;
    CGFloat posY = (FCXLoadingOffsetHeight - width) / 2;
    _activityView.frame = CGRectMake(posX, posY, width, width);
